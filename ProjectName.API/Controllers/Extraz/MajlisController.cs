...
namespace ProjectName.API.Controllers.Extraz
{
  ...
  [ApiController]
  public class MajlisController : BaseController<MajlisController>
  {
    ...
    [HttpGet]
    public async Task<IActionResult> Gets([FromQuery] PaginateRequestFilter<Majlis, CommonDtoSearch?> filter)
    {
      try
      {
        var list = await UnitOfWork.Majliss.Gets(filter);
        var result = Mapper.Map<IPagedList<Majlis>, PaginateResponse<CommonDto>>(list);
        return Ok(result);
      }
      catch (Exception ex)
      {
    ...
    {
      var single = await UnitOfWork.Majliss.Get(
        q => q.Id == id
     //, new List<string> { "Org" }
     );
    ...
    public async Task<IActionResult> Create([FromBody] CommonDtoCreate data)
    {
      ...
      try
      {
        var result = Mapper.Map<Majlis>(data);
        await UnitOfWork.Majliss.Insert(result);
    ...
    public async Task<IActionResult> Update(int id, [FromBody] CommonDtoCreate data)
    {
      ...
      try
      {
        var item = await UnitOfWork.Majliss.Get(q => q.Id == id);

        if (item == null) return UpdateNull();

        var result = Mapper.Map(data, item);
        UnitOfWork.Majliss.Update(item);
        await UnitOfWork.Save();
        return Ok(result);
      }
      catch (Exception ex)
    ...
    public async Task<IActionResult> Delete(int id)
    {
      ...
      var search = await UnitOfWork.Majliss.Get(q => q.Id == id);
      if (search == null) return DeleteNull();

      try
      {
        await UnitOfWork.Majliss.Delete(id);
        await UnitOfWork.Save();
      }
      catch (Exception ex)
      {
